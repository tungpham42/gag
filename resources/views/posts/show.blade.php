        ...
        <div class="p-6 md:p-8">
            <div class="flex items-start justify-between mb-6">
                ...
                <div x-data="{ shareOpen: false }" class="relative z-50">
                    <button @click="shareOpen = !shareOpen" @click.away="shareOpen = false" class="p-2 text-[#8C7A6B] hover:text-orange-500 dark:text-gray-400 dark:hover:text-white transition-colors rounded-xl hover:bg-orange-50 dark:hover:bg-white/5">
                    ...
                    <div x-show="shareOpen" x-transition.opacity.duration.200ms style="display: none;" class="absolute right-0 mt-2 w-48 bg-white dark:bg-[#2A2635] rounded-2xl shadow-xl border border-orange-100 dark:border-white/10 py-2 overflow-hidden">
                        ...
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('posts.show', $post)) }}" target="_blank" class="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#8C7A6B] dark:text-gray-300 hover:text-[#1877F2] dark:hover:text-[#1877F2] hover:bg-orange-50 dark:hover:bg-white/5 transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
                            Facebook
                        </a>
                        <a href="https://x.com/intent/tweet?url={{ urlencode(route('posts.show', $post)) }}&text={{ urlencode($post->title) }}" target="_blank" class="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#8C7A6B] dark:text-gray-300 hover:text-black dark:hover:text-white hover:bg-orange-50 dark:hover:bg-white/5 transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                            X (Twitter)
                        </a>
                        <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(route('posts.show', $post)) }}&title={{ urlencode($post->title) }}" target="_blank" class="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#8C7A6B] dark:text-gray-300 hover:text-[#0A66C2] dark:hover:text-[#0A66C2] hover:bg-orange-50 dark:hover:bg-white/5 transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                            LinkedIn
                        </a>
                        <a href="https://reddit.com/submit?url={{ urlencode(route('posts.show', $post)) }}&title={{ urlencode($post->title) }}" target="_blank" class="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#8C7A6B] dark:text-gray-300 hover:text-[#FF4500] dark:hover:text-[#FF4500] hover:bg-orange-50 dark:hover:bg-white/5 transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 11.5c0-1.65-1.35-3-3-3-.96 0-1.86.48-2.42 1.24-1.64-1-3.75-1.64-6.07-1.72.08-1.1.4-3.05 1.52-3.7.72-.4 1.73-.24 3 .5C17.2 6.3 18.46 7.5 20 7.5c1.65 0 3-1.35 3-3s-1.35-3-3-3c-1.38 0-2.54.94-2.88 2.22-1.43-.72-2.64-.8-3.6-.25-1.64.94-1.95 3.47-2 4.55-2.33.08-4.45.7-6.1 1.72C4.86 8.98 3.96 8.5 3 8.5c-1.65 0-3 1.35-3 3 0 1.32.84 2.44 2.05 2.84-.03.22-.05.44-.05.66 0 3.86 4.5 7 10 7s10-3.14 10-7c0-.22-.02-.44-.05-.66 1.2-.4 2.05-1.54 2.05-2.84zM2.3 11.5c0-.94.76-1.7 1.7-1.7.67 0 1.27.38 1.54.96-1.34.62-2.5 1.48-3.18 2.4-.04-.54-.06-1.1-.06-1.66zm19.4 1.66c-.68-.92-1.84-1.78-3.18-2.4.27-.58.87-.96 1.54-.96.94 0 1.7.76 1.7 1.7 0 .56-.02 1.12-.06 1.66zM12 21.5c-4.4 0-8-2.7-8-6s3.6-6 8-6 8 2.7 8 6-3.6 6-8 6zm-3.5-6.5c-.83 0-1.5-.67-1.5-1.5S7.67 12 8.5 12s1.5.67 1.5 1.5S9.33 15 8.5 15zm7 0c-.83 0-1.5-.67-1.5-1.5S14.67 12 15.5 12s1.5.67 1.5 1.5S16.33 15 15.5 15zm-8.8 1.7c.33-1 2.33-1.8 5.3-1.8s4.97.8 5.3 1.8h-10.6z"/></svg>
                            Reddit
                        </a>
